...
OUT = Path("/Volumes/rmhyw/徐嘉晟-本科毕设-sEEG-短视频-HYZ_批注修订全文副本.docx")
MD_OUT = Path("/Volumes/rmhyw/徐嘉晟-本科毕设-sEEG-短视频-HYZ_批注修订全文副本.md")
...
def next_heading_index(start_idx, max_level_prefix=None):
...
def replace_section(heading_text, blocks):
    h_idx = find_heading(heading_text)
    end = next_heading_index(h_idx)
    heading = doc.paragraphs[h_idx]
    # Preserve the heading paragraph; remove body paragraphs in this section.
...
def replace_paragraph(old_start, new_text):
    for p in doc.paragraphs:
        if p.text.strip().startswith(old_start):
            p.clear()
            p.add_run(new_text)
            return
    raise ValueError(f"paragraph start not found: {old_start}")


def add_table_after_heading(heading_text, headers, rows, caption=None):
    idx = find_heading(heading_text)
    heading = doc.paragraphs[idx]
    cursor = heading
    if caption:
        cursor = insert_after(cursor, caption)
    ...
    for row in rows:
        ...
        for j, value in enumerate(row):
            cells[j].text = value
    # Move table XML after cursor paragraph.
    tbl = table._tbl
...
# Replace abstracts.
idx_abs = find_para_exact("摘 要")
end_abs = find_para_exact("Abstract", idx_abs + 1)
for p in list(doc.paragraphs[idx_abs + 1:end_abs]):
    delete_paragraph(p)
cursor = doc.paragraphs[idx_abs]
for txt in abstract_cn:
    cursor = insert_after(cursor, txt)

idx_en = find_para_exact("Abstract")
end_en = find_para_exact("目录", idx_en + 1)
for p in list(doc.paragraphs[idx_en + 1:end_en]):
    delete_paragraph(p)
cursor = doc.paragraphs[idx_en]
...
# Add practical data tables near relevant headings.
add_table_after_heading(
    ...
    caption="表2.1 自采sEEG被试人口学信息、临床信息及纳入分析情况（待根据原始记录补全）",
)
...
add_table_after_heading(
    ...
    caption="表5.1 Keles公开数据集本地再分析的run、通道和事件数量汇总（待根据脚本输出补全）",
)

doc.save(OUT)

# Export a simple markdown text copy for quick review.
lines = []
for p in doc.paragraphs:
    text = p.text.strip()
    if not text:
    ...
    if style.startswith("Heading") or style in {"附录标识"}:
        lines.append(f"\\n## {text}\\n")
    else:
        lines.append(text)
