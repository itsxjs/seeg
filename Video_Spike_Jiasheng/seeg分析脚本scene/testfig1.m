...
%% ====== (0) 可视化高斯平滑参数（只影响画图，不影响统计/簇/Mask） ======
sigma_vis = 1.0;   % 0.5~1.5 常用；想关闭就把下面 gauss2d(...) 改回原矩阵即可
...
%% ====== (A2) 仅用于可视化的平滑版本 ======
F_val_plot   = gauss2d(F_val2, sigma_vis);
neg_plot     = gauss2d(neg2, sigma_vis);
pos_plot     = gauss2d(pos2, sigma_vis);
...
% --- (1) LME F-stat ---
subplot(1,3,1);
h1 = surf(TT, FF, F_val_plot, 'EdgeColor','none'); view(2);
...
pz = -log10(p_plot);
pz_plot = gauss2d(pz, sigma_vis);

surf(TT, FF, pz_plot, 'EdgeColor','none'); view(2);
...
subplot(1,3,3);
h3 = surf(TT, FF, F_val_plot, 'EdgeColor','none'); view(2);
set_logy(); axis tight; colorbar; colormap(gca, hot);
...
%% ---------- (1) negative ----------
subplot(1,4,1);
surf(TT, FF, neg_plot, 'EdgeColor','none');
view(2);
...
%% ---------- (2) positive ----------
subplot(1,4,2);
surf(TT, FF, pos_plot, 'EdgeColor','none');
view(2);
set_logy(); axis tight; colorbar; colormap(gca, jet);
style_tf_axes(gca);
...
hRaw = surf(TT, FF, diff_plot, 'EdgeColor','none');
view(2);
...
% 对称色标（用原始 diff2 定范围，避免平滑改变幅度造成误导）
...
hMask = surf(TT, FF, diff_plot, 'EdgeColor','none');
view(2);
set_logy(); axis tight; colorbar;
...
if isfield(res,'clusterinfo_val') && isfield(res.clusterinfo_val,'pos_clusters')
    ...
    for k = 1:numel(ci.pos_clusters)
        if ci.pos_clusters(k).p < 0.05
            ...
            % ===== NaN-safe mean（避免 mean(x,'omitnan') 的版本/路径问题）=====
            vals = diff2(mask0);
            ...
            elseif ~isnan(mean_diff)
                ...
                fprintf('    Direction: mean diff is NaN (all NaN in cluster)\n');
            end
        end
    end
end
...
%% ===== 可视化用：2D 高斯平滑（NaN-safe）=====
function Zs = gauss2d(Z, sigma)
% 对 2D 矩阵做高斯平滑（仅用于可视化）
% sigma: 建议 0.5 ~ 1.5（单位：像素）
%
% NaN-safe：NaN 不参与平滑，避免无数据污染
...
fs = 2*ceil(3*sigma)+1;
Zs = imgaussfilt(Z0, sigma, 'FilterSize', fs);
Ws = imgaussfilt(W,  sigma, 'FilterSize', fs);
