%% ===== 加载与通道选择 =====
load('sub009_valence_BOSC_TF_zpower.mat');   % 改成你的文件名
ch_label = 'POL B2';                    % 选择通道
ch = find(strcmp(chan_labels, ch_label));
if isempty(ch), error('未找到通道 %s', ch_label); end

%% ===== (1) 两条件 TFR =====
conds = {'dislike','like'};  % 顺序与 cond_mean_z 第1/2维一致
figure('Name','TFR per condition (valence)');
for c = 1:2
    subplot(1,2,c);
    Z = squeeze(cond_mean_z(c, ch, :, :));     % [freq x time]
    Z = imgaussfilt(Z, 1);                     % 轻度高斯平滑
...
%% ===== (2) 条件差图：like - dislike =====
Zdis = squeeze(cond_mean_z(1, ch, :, :));
Zlik = squeeze(cond_mean_z(2, ch, :, :));
dZ   = Zlik - Zdis;
dZs  = imgaussfilt(dZ, 1);

figure('Name','Like - Dislike');
imagesc(tms_ds, freqs, dZs); axis xy; set(gca,'YScale','log');
xlabel('Time (ms)'); ylabel('Freq (Hz)');
title('Like - Dislike (z difference)');
...
figure('Name','Band-limited time courses (smoothed, 95% CI) - valence');
for b = 1:3
