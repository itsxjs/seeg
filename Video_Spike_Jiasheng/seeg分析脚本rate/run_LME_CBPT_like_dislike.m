function run_LME_CBPT_like_dislike()
%% ===== 基本参数 =====
% 这里列出你所有已经算好 like/dislike TF 的被试
subs = {'sub001', 'sub004','sub005', 'sub007', 'sub008', 'sub009'};   % ← 修改成你自己的
data_dir = '/Users/defanive/Desktop/Diploma/seeg分析脚本rate';  % ← 放那些 subxxx_valence_BOSC_TF_zpower.mat 的地方
...
%% ===== 读第一个被试拿轴 =====
first_file = fullfile(data_dir, sprintf('%s_valence_BOSC_TF_zpower.mat', subs{1}));
S0 = load_assert(first_file, ...
    {'z_power_ds','freqs','tms_ds','chan_labels','is_like','is_dislike'});  % <<< CHANGED
freqs = S0.freqs(:)';        % 1 × F
t_vec = S0.tms_ds(:)';       % 1 × T
nF_all = numel(freqs);
nT_all = numel(t_vec);
...
sid_cell = {};
ch_cell  = {};
pref_cell = {};  % 'dislike' or 'like'   <<< CHANGED

for isub = 1:numel(subs)
    sid = subs{isub};
    infile = fullfile(data_dir, sprintf('%s_valence_BOSC_TF_zpower.mat', sid));
    S = load_assert(infile, ...
        {'z_power_ds','freqs','tms_ds','chan_labels','is_like','is_dislike'}); % <<< CHANGED
    
    zpow   = S.z_power_ds;       % [trial × chan × freq × time]
    chans  = S.chan_labels;
    is_like    = S.is_like(:);       % <<< CHANGED
    is_dislike = S.is_dislike(:);    % <<< CHANGED
    ...
    % 只分析 like 和 dislike（如果还有别的trial类型就跳过）
    use_trials = find(is_like | is_dislike);   % <<< CHANGED
    ...
    for ch = find(keep_ch)
        for tr = use_trials'
            ...
            if numel(patch)==nF*nT
                ...
                if is_like(tr)
                    pref_cell{end+1,1} = 'like';        % <<< CHANGED
                else
                    pref_cell{end+1,1} = 'dislike';     % <<< CHANGED
                end
            end
        end
    end
...
base_tbl = table( ...
    ...
    categorical(ch_cell), ...
    categorical(pref_cell), ...                    % <<< CHANGED
    'VariableNames', {'sid','ch','preference'});   % <<< CHANGED

fprintf('Collected %d observations (trial×channel)\n', nRows);

%% ===== 先算两个条件的平均，方便看方向 =====
avg_pref.dislike = nan(nF,nT);   % <<< CHANGED
avg_pref.like    = nan(nF,nT);   % <<< CHANGED
...
for f = 1:nF
    for t = 1:nT
        ...
        if any(idx_dislike), avg_pref.dislike(f,t) = mean(y(idx_dislike)); end
        if any(idx_like),    avg_pref.like(f,t)    = mean(y(idx_like));    end
    end
end
diff_map = avg_pref.like - avg_pref.dislike;  % >0 说明 like 更高  <<< CHANGED

%% ===== 逐 freq×time 做 LME: data ~ preference + (1|sid) + (1|sid:ch) =====
F_pref = nan(nF,nT);
P_pref = nan(nF,nT);
...
parfor f = 1:nF
    ...
    for t = 1:nT
        ...
        T = table( ...
            ...
            categorical(base_tbl.preference(ok)), ...   % <<< CHANGED
            double(y(ok)), ...
            'VariableNames', {'sid','ch','preference','data'});  % <<< CHANGED
        try
            mdl = fitlme(T, 'data ~ preference + (1|sid) + (1|sid:ch)', ... % <<< CHANGED
...
parfor pp = 1:nperm
    pref_perm = base_tbl.preference;   % <<< CHANGED
    for g = G
        idx = find(grp==g);
        if numel(idx)>1
...
out_name = fullfile(data_dir, 'LME_Amydgala_CBPT_2D_like_dislike_FreqTime.mat');  % <<< CHANGED
dandang(out_name, 'res','-v7.3');
fprintf('✔ Saved -> %s\n', out_name);

%% ===== 画图 =====
plot_like_dislike_results(res, data_dir);  % <<< CHANGED

fprintf('\n== DONE: like vs dislike ==\n');
end


function set_pow2_yticks(freq_vec)
% freq_vec: 你的频率轴（非均匀也没关系），用来确定范围
...
if any(res.h_pref(:))
    z0 = max(res.F_pref(:), [], 'omitnan') + 1e-6;
    BW = res.h_pref > 0;
...
saveas(fig1, fullfile(data_dir, 'LME_Amydgala_CBPT_2D_like_dislike_FreqTime_Fstats.png'));
fprintf('✔ Figure saved: LME_Amydgala_CBPT_2D_like_dislike_FreqTime_Fstats.png\n');
...
xlabel(tf_xlabel, 'FontSize', 13); ylabel(tf_ylabel, 'FontSize', 13);
title('喜欢 - 不喜欢 (方向差异)', 'FontSize', 14, 'FontWeight', 'bold');
set(gca, 'FontSize', 12, 'LineWidth', 1);

saveas(fig2, fullfile(data_dir, 'LME_Amydgala_CBPT_2D_like_dislike_FreqTime_Averages.png'));
fprintf('✔ Figure saved: LME_Amydgala_CBPT_2D_like_dislike_FreqTime_Averages.png\n');
% 打印簇信息
