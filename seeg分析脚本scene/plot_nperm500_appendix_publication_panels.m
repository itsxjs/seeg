function plot_nperm500_appendix_publication_panels()
%% Publication-style appendix panels for Study 1 nperm=500 pairwise contrasts.
...
plot_one_roi(scene_dir, out_dir, 'Amydgala', '杏仁核', ...
    'tf_appendix_study1_nperm500_amygdala_publication.png');
plot_one_roi(scene_dir, out_dir, 'Hippocampus', '海马', ...
    'tf_appendix_study1_nperm500_hippocampus_publication.png');
end

function plot_one_roi(scene_dir, out_dir, roi_prefix, roi_name, out_name)
pos_neu = load(fullfile(scene_dir, ...
    sprintf('LME_%s_CBPT_2D_pos_neu_FreqTime_nperm500.mat', roi_prefix)));
neg_neu = load(fullfile(scene_dir, ...
    sprintf('LME_%s_CBPT_2D_neg_neu_FreqTime_nperm500.mat', roi_prefix)));

contrasts = {
    prepare_res(pos_neu.res, 'pos_neu'), ...
    prepare_res(neg_neu.res, 'neg_neu')
};
avg_titles = {
    {'正性图片：平均z功率','中性图片：平均z功率','正性 - 中性（z差值）','正性 - 中性（显著簇）'}, ...
    {'负性图片：平均z功率','中性图片：平均z功率','负性 - 中性（z差值）','负性 - 中性（显著簇）'}
};
...
for c = 1:2
    row_stats = 2*c - 1;
    row_avg = 2*c;
    add_row_letter(fig, letters{row_stats}, row_stats);
    plot_stat_row(fig, contrasts{c}, layout, row_stats);
    add_row_letter(fig, letters{row_avg}, row_avg);
    plot_avg_row(fig, contrasts{c}, layout, row_avg, avg_titles{c});
end
...
function P = prepare_res(res, contrast_code)
P.freq_vec = res.freq_vec(:);
P.t_vec = res.t_vec(:)';
P.t_keep = P.t_vec >= 0 & P.t_vec <= 2000;
...
switch contrast_code
    case 'pos_neu'
        P.avg_left = res.avg_val.pos(:, P.t_keep);
        P.avg_right = res.avg_val.neu(:, P.t_keep);
    case 'neg_neu'
        P.avg_left = res.avg_val.neg(:, P.t_keep);
        P.avg_right = res.avg_val.neu(:, P.t_keep);
    otherwise
        error('Unknown contrast_code: %s', contrast_code);
end
...
set(ax, 'YScale','log', 'YLim',P.yl, 'YTick',P.yticks, ...
    'YTickLabel', compose('%g', P.yticks), 'XLim',P.xlim, ...
    'FontSize',17, 'LineWidth',1.2);
axis(ax, 'tight');
set(ax, 'XLim', P.xlim);
