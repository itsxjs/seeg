...
% ===== 条件标签（与你的 epoch 脚本一致）=====
cond_names = {'nopress','dislike','neutral','like'};
...
for i = 1:numel(subs)
    ...
    % ---- 优先加载 clean，其次 raw ----
    fn_clean = fullfile(data_dir, sprintf('%s_4bin_epoch_clean.set', subj_id));
    fn_raw   = fullfile(data_dir, sprintf('%s_4bin_epoch.set', subj_id));
...
T = table( ...
    ...
    count_mat(:,3), ...
    count_mat(:,4), ...
    'VariableNames', {'subject','total','nopress','dislike','neutral','like'});
