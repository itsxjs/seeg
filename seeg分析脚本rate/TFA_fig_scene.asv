...
out_dir  = fullfile(data_dir, 'SingleChannel_scene_QC');
if ~exist(out_dir, 'dir'), mkdir(out_dir); end

%% ====== 加载 group LME 结果（用于叠簇与簇ROI）======
L = load(fullfile(data_dir, 'LME_Amydgala_CBPT_2D_like_dislike_FreqTime.mat')); % 改成你的文件名
res = L.res;
...
for i = 1:numel(subs)
    ...
    for j = 1:numel(ch_list)
        ...
        out_png = fullfile(out_dir, sprintf('%s_%s_likeDislikeAmyd.png', sid, safe_ch));
