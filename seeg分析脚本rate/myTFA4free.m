%% ===== Bin2-locked Time-Frequency Analysis (Hilbert) =====
% 依赖：EEGLAB 已载入；已存在 <subj_id>_bin2_epoch_clean.set
% 目标：对 cutoff / finish 两类事件的 epoch 做 TFA
% 1) 逐频带带通 + Hilbert -> 功率
...
in_files  = { ...
    sprintf('%s_bin2_epoch_clean.set', subj_id), ...
    sprintf('%s_bin2_epoch.set', subj_id) ... % 兜底
};
lock_name = 'bin2';                         % 标记用
...
analysis_win = [0 2000];                    % 分析窗（bin2 epoch [-0.5 2] 内）
...
is_cut = false(n_trials,1);
is_fin = false(n_trials,1);
for tr = 1:n_trials
    et = EEG.epoch(tr).eventtype;
    if iscell(et), types = string(et); else, types = string({et}); end
    is_cut(tr) = any(strcmp(types,'cutoff'));
    is_fin(tr) = any(strcmp(types,'finish'));
end
if ~(any(is_cut) || any(is_fin))
    warning('未识别到 cutoff/finish 事件，请检查事件标注。');
end
cond_labels = {'cutoff','finish'};
...
%% ---- Step 5: 保存结果 ----
out_file = fullfile(data_dir, sprintf('%s_%s_TF_zpower.mat', subj_id, lock_name));
save(out_file, ...
