...
%% ====== 1) 路径与被试列表（你需要改这里） ======
data_dir = '/Users/defanive/Desktop/Diploma/seeg分析脚本scene';
out_dir  = '/Users/defanive/Desktop/Diploma/coh_pipeline/coherence_out';
if ~exist(out_dir, 'dir'); mkdir(out_dir); end

sub_list = {'sub005'};   % ✅ 最小可跑：先只放一个
sub_list = {'sub001','sub005','sub008'}; % 扩展时打开
...
%% ====== 3) 条件映射（事件类型 -> 你的三类） ======
cond_map = struct();
cond_map.neg = {'negative'};   % dislike = neg
cond_map.neu = {'neutral'};   % neutral = neu
cond_map.pos = {'positive'};      % like = pos
...
for si = 1:numel(sub_list)
    ...
    set_path = fullfile(data_dir, sprintf('%s_valence_epoch_clean.set', sub_id));
...
for si = 1:numel(all_sub_out)
    ...
    C_stack = cat(3, C_stack, S.coh.(dyad_name).all);
    freq_vec = S.freq_vec_hz;
    time_vec = S.time_vec_sec;
end

if ~isempty(C_stack)
    % ===== NaN-safe mean over 3rd dim（替代 mean(...,'omitnan')）=====
    X = C_stack;
    nanmask = isnan(X);
    X(nanmask) = 0;

    cnt = sum(~nanmask, 3);
    C_group = sum(X, 3) ./ max(cnt, 1);
    C_group(cnt==0) = NaN;

    % （可选）先用 group 图自动确定统一色标，再用于其它图（NaN-safe）
    vals = C_group(:);
